...
from queue import Empty, Full as queue_is_full
...
def proc_frame_reader(queue_out: Queue, config: dict, time_sleep_start: int):
    ...
    for _ in tqdm(range(time_sleep_start), desc=sleep_message):
        sleep(1)
    video_reader = VideoReader(config["video_reader"]) # генератор кадров
    for frame_element in video_reader.process():
        ts0 = time()
        try:
            queue_out.put_nowait(frame_element) # кладем кадр в очередь без блокировки
            #sleep(0.25)
            
            if PRINT_PROFILE_INFO:
                print(f"PROC_FRAME_READER: {(time()-ts0) * 1000:.0f} ms: ")

        except queue_is_full: # если очередь полна, пропускаем кадр
            if PRINT_PROFILE_INFO:
                print("queue_is_full => pass frame")

        if isinstance(frame_element, VideoEndBreakElement): # окончание потока
            break
...
def proc_proceessor(frame_queue_in: Queue, config: dict, frame_process: Process):
    ...
    while True:
        ...
        if not frame_process.is_alive(): # если процесс захвата кадров умер, выходим из цикла
            print("rendering_process is died, stopping analytics_process")
            break

        ts0 = time()
        try:
            frame_element = frame_queue_in.get(timeout=10) # ждем кадр из очереди
        except Empty: # если очередь пуста в течение 10 сек, ждем
            print("queue from camera is empty")
            continue

        ts1 = time()
        frame_element = detection_node.process(frame_element)
        frame_element = tracker_info_update_node.process(frame_element) # обновляем инфу по трекам: дороги, время жизни, удаление старых и тп
...
if __name__ == "__main__":
    ...
    check_and_set_env_var("VIDEO_SRC", "E/1.mp4")
